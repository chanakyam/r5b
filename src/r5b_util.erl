...
-export([video_db_url/0, 
		 ...
		 news_top_text_graphics_news_with_limit_skip/3,
...
news_db_url() ->
	%% http://localhost:5984/r5b/
	string:concat(?COUCHDB_URL, ?COUCHDB_TEXT_GRAPHICS)
.

news_db_url(NewsId) ->
	%% http://localhost:5984/r5b/<ID>
	string:concat(?MODULE:news_db_url(), NewsId)
.

news_top_text_news_with_limit(Category,Limit) ->
	%% http://localhost:5984/r5b/_design/news_by_category/_view/us_news?descending=true&limit=10
...
news_top_text_news_with_limit_and_skip(Limit, Skip) ->
	Url1 = string:concat(?MODULE:news_db_url(), "_design/text_us_sports/_view/by_id_title_desc_thumb_date" ),	
...
news_top_text_graphics_news_with_limit(Limit) ->
	%%http://localhost:5984/r5b/_design/news_and_graphics/_view/by_title_thumb?descending=true&limit=10
	Url1 = string:concat(?MODULE:news_db_url(), "_design/r5b/_view/by_id_title_thumb?descending=true&limit="),
	string:concat(Url1,Limit)
.

news_top_text_graphics_news_with_limit_skip(Category, Limit, Skip) ->
	Url =  string:concat(?MODULE:news_db_url(), "_design/"),
	Url_Temp =  string:concat(Url, Category),
	Url1 = string:concat(Url_Temp,"/_view/by_id_title_desc_thumb_date"),
	Url2 = string:concat(Url1, "?descending=true&limit="),
	Url3 = string:concat(Url2, Limit),
	Url4 = string:concat(Url3, "&skip="),
	string:concat(Url4, Skip)
.

news_category_url(Category, PageNumber) ->
	%% http://localhost:5984/r5b/_design/news_by_category/_view/us_news?descending=true&limit=10
	Skip = (PageNumber  -  1)  * 15,
	Url1 = string:concat(?MODULE:news_db_url(), "_design/r5b_news_by_category/_view/" ),
	Url2 = string:concat(Url1, Category),
...
news_count(Category) ->
	%% http://localhost:5984/r5b/_design/get_count/_view/<category>
...
news_by_category_limit_skip(Category, Skip, Limit) ->
	%%http://localhost:5984/r5b/_design/news_by_category/_view/us_news?limit=15&skip=0
	Url1 = string:concat(?MODULE:news_db_url(), "_design/news_by_category/_view/"),
	Url2 = string:concat(Url1, Category),
	Url3 = string:concat(Url2, "?limit="),
	Url4 = string:concat(Url3, Limit),
...
news_slideshow_url(Limit) ->
	Url1 = string:concat(?MODULE:news_db_url(), "_design/r5b/_view/by_title_view_image?descending=true&limit="),
	string:concat(Url1,Limit)
.
